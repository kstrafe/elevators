...
(require
  ; Requirements for syntax transformers
  (for-syntax racket/list racket/string racket/syntax syntax/to-string)

  ; We require ffi (foreign function interface) to call C libraries
  ffi/unsafe
...
(when (not (file-exists? library-file))
  (let-values ([(program out in err)
                (subprocess #f #f #f
                            "/usr/bin/env" "bash" "-c" (string-append "cd " (path->string this-folder) " && make"))])
    (let ([error (port->string err)])
      (when (non-empty-string? error) (ftal "Unable to build interface" error) (exit 1)))
    (port->string out)))
         ...
         [out
          `(define ,(format-symbol "~a#io" name)
             (get-ffi-obj ,(string-append "elev_" (string-replace (symbol->string name) "-" "_"))
                          ...
                          ,signature
                          (lambda ()
                            (error 'ffi-error ,(format "Unable to link to symbol ~a" (symbol->string name))))))])
